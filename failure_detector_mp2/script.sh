...
do
	...
	# sshpass -p password ssh -i ~/.ssh/googlekey ysong71illinois@$IP "sh -c 'cd ~/distributedsystem;git reset --hard; git pull; cd ~/distributedsystem/failure_detector_mp2;make clean; make DEBUG=-DDEBUG; nohup ./failure_detector.out > /dev/null 2>&1 &'"
