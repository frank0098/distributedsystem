...
do
	echo "====="$IP"======"
	sshpass -p password ssh -i ~/.ssh/googlekey ysong71illinois@$IP "sh -c 'cd ~/distributedsystem/failure_detector;\
	git pull;make clean; make DEBUG=- DDEBUG; nohup ./failure_detector.out > /dev/null 2>&1 &'"
